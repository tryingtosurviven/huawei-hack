...
function extractText(node) {
  // Finds text inside the DM bubbles
  const el = node.querySelector('div[dir="auto"] span, .x1n2onr6 span, div[role="none"]');
  return el?.innerText?.trim() || null;
}

function getMessageContainers(node) {
  // Targets the common container for DM bubbles
  return node.querySelectorAll('.x1n2onr6, div[role="row"]');
}
